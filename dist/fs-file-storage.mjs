...
var index = (function (dataPath) {
  var indexPath = path.join(dataPath, 'indexs');
  var contentsPath = path.join(dataPath, 'contents');
  /**
   * 初始文件目录
   */
  ...
  var get = function (filterJson) {
    try {
      if (typeof filterJson === 'string') {
        return Promise.resolve(readJson(join(contentsPath, filterJson)));
      }
      ...
      return Promise.resolve(getList()).then(function (list) {
        ...
        return item ? readJson(join(contentsPath, item._id)) : null;
      });
    } catch (e) {
      return Promise.reject(e);
    }
      ...
      return Promise.resolve(init()).then(function () {
        return Promise.resolve(getList()).then(function (list) {
          ...
          return Promise.resolve(writeJson(indexPath, list)).then(function () {
            return Promise.resolve(writeJson(join(contentsPath, id), Object.assign({}, newJson,
      ...
      return Promise.resolve(get(filterJson)).then(function (item) {
        return Promise.resolve(getList()).then(function (list) {
          ...
          if (replace) {
            ...
            writeJson(join(contentsPath, id), Object.assign({}, json,
              infoJson,
              {_id: id}));
            return;
          }
          ...
          list.splice(index, 1, Object.assign({}, list[index],
            json));
          writeJson(indexPath, list);
          writeJson(join(contentsPath, id), Object.assign({}, item,
            json,
            infoJson));
        });
      });
    ...
    try {
      return Promise.resolve(init()).then(function () {
        return Promise.resolve(getList()).then(function (list) {
          ...
          writeJson(indexPath, list);
          writeJson(join(contentsPath, id), Object.assign({}, newJson,
            infoJson));
          return list.length;
        });
      });
